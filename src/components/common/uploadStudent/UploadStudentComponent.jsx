...
function UploadStudentComponent ({
	...
	let layoutTwo = (
		<div>
			...
			<div className={styles.check_import_parent}>
			    <Checkbox checked={importParent} onChange={changeImportParent}>同时导入联系人信息</Checkbox>
			</div>
	...
	return (
		<div>
			<Button type="default" onClick={() => onClickUploadButton()} style = {{ marginRight : (!Changecolor ? '10px':'0')  , background : '#88C702' , border : '1px solid #88C702' , color : '#fff'}}>{!Changecolor ? '批量导入':'导入付费客户' }</Button>
			<Modal
				width={1000}
				title="导入付费客户"
				visible={showModal}
