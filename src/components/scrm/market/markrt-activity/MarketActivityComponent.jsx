...
import {Button, Form, Popconfirm, Modal, Input, DatePicker, Select, Checkbox, InputNumber, Popover} from 'antd';
...
function MicroModuleFormComponent ({
	...
	function reviewDetail(record) {
		...
		dp('getAnalysisData', {showAnalysisModal : !showAnalysisModal, orgId : record.orgId, actId : record.id, createTime :  record.createTime});
		dp('summaryQuery', {} );
	}
		  		...
		  		<Form>
					...
					<FormItem {...formItemLayout} label="采集地点">
				  		{getFieldDecorator('activitySecondSource', {
							...
							<Select placeholder="请选择采集地点" notFoundContent="请在设置-系统设置-业务参数-二级来源中添加采集地点" disabled={editStatus}>
								{didianChildren}
							</Select>
						)}
					</FormItem>
			  	...
			  	<div className={styles.modalButtonDivStyle}>
					<Button className={styles.modalButtonStyle} onClick={() => addActivityHandleCancel()}>取消</Button>
				  	<Button className={styles.modalButtonStyle} 
				  		...
				  		disabled={editStatus ? saveButtonDsiabled : disabledCreateQrBtn}
