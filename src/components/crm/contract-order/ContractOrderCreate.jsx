...
function ContractOrderCreate({
    ...
    function CustomerIdChangeAction(value){
        if(!!getFieldValue('parentId')){
            setFieldsValue({
                parentId : undefined
            });
        }
				...
				if( !!teachToolsArr ){
					for( let i = 0; i < teachToolsArr.length; i++ ){
						...
						if( !getFieldValue('classPackage_' + item.key )){
							message.error( '请选择增值服务; 如果不需要请删除增值服务输入项' );
							return
						}
						teachTools.push({
            ...
            <Form>
				...
				>
					{ getFieldDecorator('orderName', {
						initialValue : contractOrderInfo.orderName || undefined,
						rules : [
							{ required : true, message : '请输入合同名称' }
						]
					})(
			   			<Input disabled = { editDisabled } size = 'default' placeholder = { '请输入合同名称' } style = {{ width : '308px' }} />
					)}
				</FormItem>
				<FormItem
					{ ...formItemLayout }
				...
				>
					{ getFieldDecorator('orderStatus', {
						initialValue : contractOrderInfo.orderStatus || undefined,
					})(
			   			<Input disabled = { editDisabled } size = 'default' placeholder = { '请输入合同状态' } style = {{ width : '308px' }} />
					)}
				</FormItem>
				<FormItem
					label = '签约客户'
				...
				>
					{ getFieldDecorator('stuId', {
						initialValue : contractOrderInfo.stuId || undefined,
						rules : [
							{ required : true, message : '请选择签约客户' }
						]
					})(
						<Select
                            showSearch
                            size = 'default'
							placeholder = '请先选择签约客户，再选择联系人'
                            notFoundContent = "没有签约客户"
							optionFilterProp = 'children'
							style = {{ width : '308px' }}
							onChange = { CustomerIdChangeAction }
						>
							{ customerList && customerList.map(function( item, index ){
								return ( <Option key = { 'contract_order_parent_' + item.id } value = { item.id } >{ item.name || '' }</Option>)
							}) }
						</Select>
					)}
				</FormItem>
			...
			<Form>
				...
				{ getFieldValue('orderType') == '2' &&
					<FormItem
						style = {{ marginTop : '20px' }}
						label = '赠送总成本'
                        help = '(用于统计赠送消耗的成本，不计入合同总额，也不计入消课统计)'
						{ ...formItemLayout }
					>
					...
					<div style = {{ position : 'relative' }}>
						...
						>
							{ getFieldDecorator('extPeriodReason', {
								...
								<Input size = 'default' placeholder = '请输入赠送原因, 限20字' style = {{ width : '308px' }} />
							)}
						</FormItem>
                        {/*<p className = { styles.quick_send_class }>
							<span>快速选择 : </span>
							<span className = { styles.quick_send_class_item } onClick = { () => setIntoExtPeriodReason( '产品赠送' ) } >产品赠送</span>
				...
				<FormItem
					{ ...formItemLayout }
					label = '合同附件'
					help = '最多10张, 支持png, jpeg, gif格式的图片, 不大于5M'
                    className = 'zj_contract_upload_pic'
				>
...
//						]
						...
						>
							{ getFieldValue('imgUrl') && getFieldValue('imgUrl').length >= 10 ?  null : uploadButton }
						</Upload>
					)}
				</FormItem>
                <FormItem
					...
					>
						{ getFieldDecorator('payType', {
							initialValue : contractOrderInfo.payType || undefined,
                            rules : [
								{ required : true, message : '请输入付款方式' }
							]
						})(
							<Input size = 'default' style = {{ width : '308px' }} placeholder = '请输入付款方式'/>
						)}
				</FormItem>
                <FormItem
						style = {{ marginTop : '20px' }}
					...
					>
						{ getFieldDecorator('payName', {
							initialValue : contractOrderInfo.payName || undefined,
                            rules : [
								{ required : true, message : '请输入付款方名称' }
							]
						})(
							<Input size = 'default' style = {{ width : '308px' }} placeholder = '请输入付款方名称'/>
						)}
				</FormItem>
                <FormItem
						style = {{ marginTop : '20px' }}
					...
					>
						{ getFieldDecorator('returnMoney', {
							initialValue : contractOrderInfo.returnMoney || undefined,
                            rules : [
								{ required : true, message : '请输入回款' }
							]
						})(
							<Input size = 'default' style = {{ width : '308px' }} placeholder = '请输入回款'/>
						)}
				</FormItem>
				<FormItem
					label = '备注'
