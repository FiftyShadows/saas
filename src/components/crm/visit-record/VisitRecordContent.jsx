			...
			<div className = 'follow_record_content_right'>
				...
				<div className = 'card_detail_record'>
					...
					<div className = 'card_btn'>
						<Popconfirm onConfirm = { () => confirmVisited( selectedItem ) } okText = '确认' cancelText = '取消' title = '确认到访么?' >
							<Button disabled = { status == '1' || status == '0' || dataSource.length == 0 } type = 'primary' >
								确认到访
							</Button>
						</Popconfirm>
						<Button disabled = { status == '1' || status == '0' || dataSource.length == 0 } style = {{ marginLeft : '20px' }} type = 'primary' onClick = { () => updateVisitRecord( selectedItem ) }>
							编辑
						</Button>
						<Popconfirm onConfirm = { () => cancelVisitRecord( selectedItem ) } okText = '确认' cancelText = '取消' title = '确认取消么?' >
							<Button disabled = { status == '1' || status == '0' || dataSource.length == 0 } style = {{ marginLeft : '20px' }} >取消</Button>
						</Popconfirm>
					</div>
				</div>
			</div>
