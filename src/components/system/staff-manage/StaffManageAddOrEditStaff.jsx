...
const StaffManageAddOrEditStaff = ({
    ...
    /*账号所属内容改变时(为了清空角色下拉列表)*/
    let AccountContentOnChange = function(data){
        setFieldsValue({
            leaderId: undefined,
        });
        StaffManageAddOrEditChooseOrg(data);        //选择账号所属的onChange事件，用来发送请求，获取汇报对象下拉列表
    }
    ...
    return (
        <Modal {...modalOpts}>
            ...
            { addOrEditStaffModalType != 'edit' ?
                <div>
                    <BlockTitle content = '职能信息' style = {{ marginBottom : 5 }}/>
                    <div style = {{ color : 'red' , marginLeft : 18 }}>若该账号为系统总管理员账号，以下内容仅供参考，禁止修改</div>
