                    ...
                    if( wetherRoleItemChooseIndex == index ){
                        ...
                        return(
                            <div key={item.id} className={styles.roleNameUnCheck} onClick={() => CheckRoleFunction(item,index)}>
                                <span style={{marginLeft:'5px'}}>{item.name}</span>
                                <Popconfirm placement="top" title={<span>删除将导致该角色不可用，确定要删除<strong style={{color:'red'}}>{item.name}</strong>吗？</span>} onConfirm={() => DeleteRoleItem(item.id)} okText="是" cancelText="否">
                                    <a style={{float:'right',marginLeft:'20px',fontSize:'1.16rem',marginRight:'5px'}}>删除</a>
                                </Popconfirm>
                                <a onClick={() => CopyRoleItem(item.id,item.name)} style={{float:'right',marginLeft:'20px',fontSize:'1.16rem'}}>复制</a>
                                <a onClick={() => RenameRoleName(item,index)} style={{float:'right',marginLeft:'20px',fontSize:'1.16rem'}}>重命名</a>
                            </div>
    ...
    const loopAllFunctionList = data => data.map((item) => {
        if (item.list) {
            if(item.mainTitle == true){
                return <TreeNode title={<span style={{fontSize:'1.33rem',color:'#666666',lineHeight:'20px'}}>{item.name}</span>} key={item.id} >{loopAllFunctionList(item.list)}</TreeNode>;
            }else{
                return <TreeNode title={<span style={{fontSize:'1.16rem',color:'#999999',lineHeight:'18px'}}>{item.name}</span>} key={item.id} >{loopAllFunctionList(item.list)}</TreeNode>;
            }
        }else{
            return <TreeNode title={<span style={{fontSize:'1.16rem',color:'#999999'}}>{item.name}</span>} key={item.id} />
        }
    });
