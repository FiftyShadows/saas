import {
    ...
    GetStudentTypeList,     //得到机构类型下拉列表
    GetDictMessage,         //获取数据字典数据(跟进状态，一级来源)下拉列表内容
    GetOrgStaffMessage,     //获取机构下的员工下拉列表内容
    GetRecommend,           //获取推荐人(联系人)下拉列表内容
    AddLeadsOnBlurCheckSame,//孩子姓名，联系人姓名，联系人手机号onblur查重
    GetNameFormThisOrg,     //在全部leads中查询当前校区下这个名称的学员
    GetPhoneFormThisOrg,    //在全部leads中查询当前校区下这个电话的学员
    AddNewLeads,            //点击新增保存
...
export default {
    ...
    state: {
        ...
        //名单电话查重
        checkSamePhone : false,                     //是否重复,false则不能新增名单
            ...
            history.listen(( { pathname, query }) => {
                if(pathname == '/crm_leads_add'){
                    dispatch({
                        ...
                        payload:{
                            leadsAddType : '1',
                            leadsImportModalOrgId : (window._init_data.firstOrg).key,
                        }
                    });
                    //可以获取当前用户的ID
                    dispatch({
    ...
    effects: {
        ...
        *'GetConnectStatus'({ payload },{ put , call , select }){
            ...
            if(!!res && res.ret && res.ret.errorCode == '9000'){
                ...
                yield put({
                    type : 'updateState',
                    payload : {
                        leadsAddConnectState : ret.list || [],                  //联系方式
                    }
                })
            }else{
        ...
        *'GetOrgScale'({ payload },{ put , call , select }){
            ...
            if(!!res && res.ret && res.ret.errorCode == '9000'){
                ...
                yield put({
                    type : 'updateState',
                    payload : {
                        leadsAddOrgScale : ret.list || [],                       //机构规模
                    }
                })
            }else{
                message.error(!!res && res.ret && res.ret.errorMessage ? res.ret.errorMessage : '获取联系状态失败')
        ...
        *'GetFollowType'({ payload },{ call, put, select }){
        ...
        //得到机构类型下拉列表
        *'GetStudentTypeList'({ payload },{ call, put, select }){
            let { ret } = yield call(GetStudentTypeList,parse(payload));
            if (ret && ret.errorCode == '9000') {
                yield put({
                    type: 'updateState',
                    payload: {
                        leadsAddStuType : ret.list
                    }
                })
            } else {
        ...
        *'GetNameFormThisOrg'({ payload },{ call, put, select }){
        ...
        //在全部leads中查询当前校区下这个电话的学员
        *'GetPhoneFormThisOrg'({ payload },{ call, put, select }){
            yield put({ type:'showLoading' });
            let { ret } = yield call(GetPhoneFormThisOrg,parse(payload));
            if(ret && ret.errorCode === 9000){
                if(!!ret.flag){
                    message.success('没有重复');
                }else{
                    let sellerName = ret.sellerName;
                    let name = ret.name;
                    if(sellerName != '' && sellerName ){
                        message.error('该客户已存在，负责销售：'+sellerName+'，账号姓名 ：'+name);
                    }else{
                        message.error('该客户已存在,暂无负责销售人员');
                    }
                ...
                yield put({
                    type : 'updateState',
                    payload : {
                        checkSamePhone : ret.flag
                    }
                })
            }else{
                //接口报错则默认可以提交
                message.error(ret && ret.errorMessage ? ret && ret.errorMessage : '查重失败');
                yield put({
                    type : 'updateState',
                    payload : {
                        checkSamePhone : false
                    }
                })
            }
            yield put({ type:'closeLoading' });
