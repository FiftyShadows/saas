import {
	...
	getParentIdList,              //校区得到联系人下拉列表
	getVipCardList,               //联系人得到会员卡下拉列表
	getStuIdList,                 //联系人得到学员下拉列表
    getCustomerIdList,            //获取客户下拉列表
...
export default {
  	...
  	effects: {
		...
		*openContractOrderModal({ payload },{ call, put, select }){
            ...
            /*得到客户信息*/
			let customerList = yield call( getCustomerIdList, ({ orgId : orgId }));
			if( customerList && customerList.ret && customerList.ret.errorCode == 9000 ){
				yield put({
					type : 'updateState',
					payload : {
						customerList : customerList.ret.result,
					}
				})
			}else{
				message.error( customerList && customerList.ret && customerList.ret.errorMessage || '客户下拉获取失败' )
			}
        ...
        *'getParentIdList'({ payload },{ call, put, select }){
            /*得到校区下的联系人*/
			let { ret } = yield call( getParentIdList, parse(payload));
			if( ret && ret.errorCode == 9000 ){
				yield put({
					type : 'updateState',
					payload : {
				...
				message.error( ret && ret.errorMessage || '联系人下拉获取失败' )
			}
        },
		...
		*TenantSelectOnSelect({ payload },{ call, put, select }){
			let { value } = payload;
			let { ret } = yield call( getCustomerIdList, ({ orgId : value }));
			if( ret && ret.errorCode == 9000 ){
				yield put({
					type : 'updateState',
					payload : {
		...
		*confirmAddContractOrder({ payload },{ call, put, select }){
			...
			if( values.orderType == '1' ){
				let params = {
					...
					signTime  : values.signTime && values.signTime.format('YYYY-MM-DD HH:mm:ss')
				}
				let ret = {};
				if( !!orderId ){
