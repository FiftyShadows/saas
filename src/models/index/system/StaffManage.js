    ...
    effects: {
        /*组织结构*/
            ...
            *'AddSector'({ payload }, { call, put, select }){
                ...
                if (ret && ret.errorCode === 9000) {
                    ...
                    let pids = JSON.parse(payload.pids);
                    for(let i in pids){
                        (payload.secondOrganArray).push(pids[i]+'')
                    }
                    yield put({
        ...
        /*员工列表*/
            ...
            *'ShowStaffTable'({ payload }, { call, put, select }){
                yield put({ type:'showRightTableLoading' });
                let { ret } = yield call(ShowStaffTable,parse(payload));
                if (ret && ret.errorCode === 9000) {
                    ...
                    if(payload.status == '1'){
                        yield put({
                            ...
                            payload: {
                                ...
                                staffManagePageSizeUseing : ret.data.pageSize
                            },
                        });
                        ...
                        yield put({
                            type: 'updateState',
                            payload: {
                                staffManagePageIndexDisabled : ret.data.pageIndex,         //已停用表格页码
                                staffManagePageSizeDisabled : ret.data.pageSize,           //已停用表格每页显示条数
                            },
                        });
                    }
                }else if( ret && ret.errorMessage ){
            ...
            *'GetStaffDetail'({ payload }, { call, put, select }){
                ...
                if (ret && ret.errorCode === 9000) {
                    ...
                    if((ret.data).roles && ((ret.data).roles).length > 0){
                        let campus = [];
                        for(let i in ret.data.mgrOrgs){
                            campus.push(ret.data.mgrOrgs[i].org_id + '');
                        }
                        if(((ret.data).roles)[0].role_key == 'admin'){
                            yield put({
                                type: 'updateState',
                                payload: {
                                    wetherSystemMgr : true,                 //是系统管理员
