  ...
  namespace: 'leadsFollowSheet',
    ...
    effects: {
        ...
        *'QueryFollowRecord'({ payload },{ call, put, select }){
            ...
            let { ret } = yield call(QueryFollowRecord,parse(params));
            if (ret && ret.errorCode === 9000) {
                ...
                ret && ret.errorMessage ? message.error(ret.errorMessage) : message.error('跟进记录查询失败');
                yield put({
                    type:'updateState',
                    payload:{
        ...
        *'QuerySalesFunnelPlot'({ payload },{ call, put, select }){
            ...
            delete payload.exportSearchContent;
            let params = { ...payload , ...exportSearchContent };
            let { ret } = yield call(QuerySalesFunnelPlot,parse(params));
            if (ret && ret.errorCode === 9000) {
                yield put({
                    type:'updateState',
                    payload:{
                ...
                ret && ret.errorMessage ? message.error(ret.errorMessage) : message.error('销售漏斗查询失败');
                yield put({
                    type:'updateState',
                    payload:{
        ...
        *'QueryReservationRadar'({ payload },{ call, put, select }){
            ...
            delete payload.exportSearchContent;
            let params = { ...payload , ...exportSearchContent };
            let { ret } = yield call(QueryReservationRadar,parse(params));
            if (ret && ret.errorCode === 9000) {
                yield put({
                    type:'updateState',
                    payload:{
                ...
                ret && ret.errorMessage ? message.error(ret.errorMessage) : message.error('预约试听查询失败');
                yield put({
                    type:'updateState',
                    payload:{
        ...
        *'QueryVisitedRadar'({ payload },{ call, put, select }){
            ...
            delete payload.exportSearchContent;
            let params = { ...payload , ...exportSearchContent };
            let { ret } = yield call(QueryVisitedRadar,parse(params));
            if (ret && ret.errorCode === 9000) {
                yield put({
                    type:'updateState',
                    payload:{
                ...
                ret && ret.errorMessage ? message.error(ret.errorMessage) : message.error('到访记录查询失败');
                yield put({
                    type:'updateState',
                    payload:{
                        visitedSheetData : [],
