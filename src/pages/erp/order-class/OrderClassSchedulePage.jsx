...
class OrderClassSchedulePage extends React.Component {
	...
	querySchedule( startDate, endDate, orgId ){

		let me = this;

        serviceRequest(`${BASE_URL}/cerpCoursePlan/query`, { startDate, endDate, orgId , pageIndex : 0 , pageSize : 99999 },
            function( ret ) {
                let state = me.state;
				me.setState({
	...
	changeTimeByDayAndWeek( type ){
		...
		this.querySchedule( newStartDate, newEndDate, orgId );
	}

	//点击切换按天按周展示
	dayAndWeekChange( e ){
		let { orgId } = this.state;            //得到orgId
