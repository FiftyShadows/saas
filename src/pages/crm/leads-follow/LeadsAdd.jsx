...
function LeadsAdd({ dispatch , leadsAdd }) {
    ...
    //孩子姓名，联系人姓名，手机号输入框onBlur事件，用来即时查重
    ...
    //孩子姓名点击查重(orgId,name)
    function LeadsAddCheckSameName(orgId,tureName){
        if(orgId == undefined || orgId == '' || orgId == null || /^[\s]*$/.test(orgId)){
            message.warn('请选择校区')
        }else if(tureName == undefined || tureName == '' || tureName == null || /^[\s]*$/.test(tureName)){
    ...
    //电话点击查重(orgId,name)
    function LeadsAddCheckSamePhone(orgId,mobile){
        if(orgId == undefined || orgId == '' || orgId == null || /^[\s]*$/.test(orgId)){
            message.warn('请选择校区')
        }else if(mobile == undefined || mobile == '' || mobile == null || /^[\s]*$/.test(mobile)){
            message.warn('请输入电话')
        }else{
            dispatch({
                type:'leadsAdd/GetPhoneFormThisOrg',
                payload:{
                    orgId,
                    mobile : (mobile + '').trim(),
                }
