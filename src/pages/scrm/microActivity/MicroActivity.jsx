...
const MicroActivity = React.createClass({
    ...
    zyfOnSroll(){
        ...
        if(div.clientHeight + div.scrollTop >= div.scrollHeight && div.scrollTop > 0 && !this.state.wetherScrollBottom){
            ...
            let organId = this.state.organId ;
            let labelIds = [];
    ...
    render() {
        ...
        labelList = this.state.myLabel.map(function(item,index){
            ...
            let div = labelValue.map((valueLabel,indexLabel) => {
                ...
                return  (
                    <div key = {'label_'+index} className={'labeGroup'}>
                        <div className={'labeTitle'}>{item.group}：</div>
                        <div className={'labeContent'}>
                            { div || [] }
                        </div>
                    </div>
                )
		...
		if( self.state.acitvityList && self.state.acitvityList.length > 0 ) {
			acitityList = self.state.acitvityList.map(function (item, index) {
				...
				let delay = index * .1;
	        	return <div style = {{ animationDelay : delay + 's' }} key = { "microActivity_" + index } className = 'activity_div' >
							<div className="activity_div_img" style={{backgroundImage : imageurl, backgroundSize: '100% 100%', backgroundPosition: 'center'}}>
								<div className="general_use_number"></div>
								<p className="general_use_number_p_left">家机构已经创建<span className="general_use_number_p_right">{viewNum} </span></p>
							</div>
							<div className="activity_div_botton_div">
								<p className="activity_div_title">{item.title}</p>
								<p className="activity_div_sub_title">{item.intro}</p>
								...
								<p className="Oractivity_div_open_status">{item.isopen == '1' ? '' : "￥"+price }</p>
							</div>
							<div className = 'codeModalActive' >
								<div className = 'codeUrlActive' >
    	...
    	return (
    			...
    			<div className="game_general_base"
                    ...
                    <Spin spinning = {this.state.loading}>
                        <div className='zyf_micro_activity_search_ontent'>
                            <div className='zyf_micro_activity_search_item'>
                            ...
                            <div className='zyf_micro_activity_search_button_group'>
                        ...
                        <div className="active_clearance"></div>
						...
						<div className = "active_main">
							{this.state.myLabel.length >3 ?
								<div className = "zyf_micro_activity_check_or_pickup_more_tag" onClick = {() => this.checkMoreOrPickup(this.state.checkOrPickup)}>
                                { this.state.checkOrPickup == 'check' ?
                                    <div>
                                        <div className='zyf_micro_activity_check_or_pickup_more_tag_font'>更多</div>
                                        <Icon type="double-right" className='zyf_micro_activity_check_more_tag'/>
                                    </div> :
                                  this.state.checkOrPickup == 'pickup' ?
                                    <div>
                                        <div className='zyf_micro_activity_check_or_pickup_more_tag_font'>收起</div>
                                        <Icon type="double-right" className='zyf_micro_activity_pickup_more_tag'/>
                                    </div> : ''
								}
							</div>
								:''
							...
							<div className="general_game_acitity_list" data-name = 'microActivity'>
								{ acitityList }
							</div>
