...
function StuSilenceSheetPage({ dispatch, stuSilenceSheetModel }){
	...
	function exportUnPackFunc(){
		let exportPath = `${BASE_URL}/erpStatsSilence/purExport?${qs.stringify(exportSearchContent)}`;
	...
	function exportUnCostFunc(){
		let exportPath = `${BASE_URL}/erpStatsSilence/costExport?${qs.stringify(exportSearchContent)}`;
        if( !uncostStuNum ){
            return message.warn('无查询结果可导出');
        }else{
			exportData( exportPath )
